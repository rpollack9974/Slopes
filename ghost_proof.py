class ghost_proof(ghost):
		...
		for comp in range(self.p-1):
			for j in range(num_coefs+1):
				for z in self.zeroes(comp,j):
					...
					self.zero_data[(comp,j,z)] = [1 for a in range(self.mult(comp,j,z))] 
	...
	def close_to_ghost_zero(self,comp,j,k,z):
		p = self.p
		return (ZZ(k)-ZZ(z)).valuation(p)+1 >= min(self.zero_data[(comp,j,z)])
	...
	def good_weight(self,comp,j,k,verbose=false):
		...
		for z in self.zeroes(comp,j):
			if self.zero_data.has_key((comp,j,z)):
				bool2 = (ZZ(k)-ZZ(z)).valuation(p)+1 < min(self.zero_data[(comp,j,z)])
				bool = bool and bool2
	...
	def y_lowerbnd(self,comp,j,k):
		...
		for kss in self.zeroes(comp,j):
			if not self.close_to_ghost_zero(comp,j,k,kss):
				...
				for bnd in self.zero_data[(comp,j,kss)]:
		...
		while m>=r:
			...
			if sample[0]:
				...
				if sample[1] < bnd:
					...
					self.zero_data[(comp,j,z)][0] = (ZZ(k)-ZZ(z)).valuation(p) + 1.0001
					return True
			else:
				ks[ind] = z+1
				v = [(ZZ(k)-ZZ(z)).valuation(p)+1 for k in ks]
	...
	def are_zeroes_separated(self,comp,j,verbose=False):
		...
		while bool and a<len(self.zeroes(comp,j))-1:
			bool = bool and min(self.zero_data[(comp,j,z)]) > self.sep_ghost_zero(comp,j,z)
		...
		if not self.are_zeroes_separated(comp,j):
			...
			for z in zs:
				if self.mult(comp,j,z) == 1:
					...
					self.zero_data[(comp,j,z)] = [self.y_lowerbnd(comp,d,z) + (j-d) * (z-2)/2 - away_from_z]
			return True
